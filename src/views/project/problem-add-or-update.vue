  ...
  >
    <div class="host-add-or-update">
      <el-form ref="dataForm" :model="dataForm" label-width="80px" :rules="rules">
        ...
        <el-form-item label="解决时间" prop="solve_time" :formatter="timeFormatter">
          <el-date-picker
...
export default {
  ...
  methods: {
   init(data){
    ...
    timeFormatter(row){
      return row.solve_time.substr(0,19).replace("T"," ")
    },
     submitData(){
      this.$refs['dataForm'].validate(valid=>{
        if(valid){
          this.loading=true;
          let url=this.type=='post'?'/test/question':`/test/question/${this.dataForm.id}`
          this.$http({
            url:url,
            method:this.type,
            data:{
